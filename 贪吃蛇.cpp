#include <windows.h>  //windows环境下各种函数库（图像 声音等）
 #include <stdlib.h>
 #include <time.h>    //从机器获取时间的函数库
 #include <stdio.h>
 #include <string.h>
 #include <conio.h>  //用户控制台输入输出函数库
 #define N 21 
        ...
        for (i = 0; i < N + 2; ++i) 
                 ...
                 for (j = 0; j < N + 2; ++j) 
                         ...
                         switch (plate[i][j]) 
                                 {
                              case 0: 
                                  color(12);printf("F");color(11); continue;
                         case 1: printf("D"); continue;
                              default: ;
                              }
                     }
                    putchar('\n');
